...
## osascript -e "tell app \"System Events\" to display dialog \"$1\" buttons {\"OK\"} with title \"DOSBox-X Printing\""
...
on run argv
	...
	set pdfPath to argv
	set pdfPosix to pdfPath & "/selectraw.prn"
	set bonjourPosix to (pdfPath & "BonjourEvents.app") as string
	set msgTitle to "DOSBox-X Printing"
	
	(*
	tell application "System Events"
			activate
			display dialog bonjourPosix buttons {"OK"} with title msgTitle
	end tell
	*)
	...
	if the (count of printerList) is 1 then
		...
		if thePrinter is false then
			...
			*)
			...
			try
				tell application (pdfPosix & "BonjourEvents.app")	
		...
		*)
			...
			try
				do shell script "lpr -rlP " & "\"" & theQueue & "\"" & " " & "\"" &  pdfPosix & "\""
			on error err
				tell application "System Events"
					activate
					display dialog err & return & return & "Could not send raw output to printer." buttons {"OK"} with title msgTitle giving up after 10
				return
				end tell
			end try
